//dataの引数を追加
class AreaChart1 {
    constructor (config, data_price, data_rate) {
        this.config = {
    ...
    update() {
        let self = this;
        console.log(self.data_rate)
        console.log(self.data_price)

        self.xscale1.domain([
          // データ内の日付の最小値を取得
          d3.min(self.data_price, function(d){return d.date;}),
          // データ内の日付の最大値を取得
          d3.max(self.data_price, function(d){return d.date;})
        ]);
        ...
        self.yscale1.domain([
          ...
          0,
          // データ内のvpriceの最大値を取得
          d3.max(self.data_price, function(d){return d.price;})
        ]);
        self.yscale1_rate.domain([
          // 0を最小値として設定
          0,
          // データ内のexchangerateの最大値を取得
          d3.max(self.data_rate, function(d){return d.exchangerate;})
        ]);
